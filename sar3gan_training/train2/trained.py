...
class GeneratorStage(nn.Module):
    def __init__(self, InputChannels, OutputChannels, Cardinality, NumberOfBlocks,
        ...
        self.ResamplingFilter = ResamplingFilter

        # Transition: Conv if first stage, Upsample otherwise
        ...
        # Self-Attention: **input channels depend on whether we apply before transition**
        if ResamplingFilter is not None:
            self.Attention = SelfAttention(InputChannels)  # attention before upsample
        else:
            self.Attention = None
    ...
    def forward(self, x):
        ...
        # Apply attention **only before UpsampleLayer**
        if self.ResamplingFilter is not None and self.Attention is not None and x.ndim == 4 and x.shape[-1] == 64:
            x = self.Attention(x)

        # Apply Transition (Conv or Upsample)
        x = self.Transition(x)
...
class DiscriminatorStage(nn.Module):
    def __init__(self, InputChannels, OutputChannels, Cardinality, NumberOfBlocks, ExpansionFactor, KernelSize, VarianceScalingParameter, ResamplingFilter=None, DataType=torch.float32):
        ...
        # Self-Attention: initialized with OutputChannels if downsample will be applied
        self.Attention = SelfAttention(OutputChannels if ResamplingFilter is not None else InputChannels)
    ...
    def forward(self, x):
        ...
        # Apply residual blocks first
        for block in self.Blocks:
            x = block(x)

        # Apply transition (downsample or adaptive pool + linear)
        if isinstance(self.Transition, nn.AdaptiveAvgPool2d): 
            x = self.Transition(x).view(x.size(0), -1) 
            x = self.Linear(x) # becomes 2D
        else:
            x = self.Transition(x)

        # Self-Attention exactly here, conditional on 64x64
        if x.ndim == 4 and x.shape[-1] == 64:
            x = self.Attention(x)
